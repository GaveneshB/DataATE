// Pickup Form JavaScript - FINAL FIXED VERSION
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Pickup form initialized');
    initializeForm();
});
...
// Setup file input listeners - CRITICAL FIX
function setupFileInputs() {
    const carImagesInput = document.getElementById('carImages');
    const agreementInput = document.getElementById('agreementForm');
    
    if (carImagesInput) {
        console.log('Car images input found');
        carImagesInput.addEventListener('change', function(e) {
            console.log('Car images selected:', this.files.length);
            handleImageUpload(this, 'carImagesPreview');
        });
    }
    
    if (agreementInput) {
        console.log('Agreement input found');
        agreementInput.addEventListener('change', function(e) {
            console.log('Agreement file selected:', this.files[0]?.name);
            handleFileUpload(this, 'agreementPreview');
        });
    }
}
    ...
    files.forEach((file, index) => {
        ...
        reader.onload = function(e) {
            ...
            if (processedCount === files.length) {
                console.log('All images processed:', carImages.length);
...
function renderImagePreview(container) {
    ...
    console.log('Rendering', carImages.length, 'images');
...
function handleFileUpload(input, previewId) {
    ...
    // Save file
    agreementFile = file;
    console.log('Agreement file saved:', agreementFile.name);
...
function removeFile(previewId) {
    console.log('Removing file from:', previewId);
    agreementFile = null;
...
function handleSubmit(e) {
    ...
    if (!agreementFile) {
        showNotification('Please upload the agreement form', 'error');
        return;
    }
    ...
    // Remove any existing agreement_form entries
    formData.delete('agreement_form');
    
    // Add agreement file
    formData.append('agreement_form', agreementFile);
    ...
    if (submitBtn) {
        ...
        setTimeout(() => {
            ...
            showNotification('Pickup form submitted successfully!', 'success');
            ...
            setTimeout(() => {
                showNotification('Redirecting to completion page...', 'info');
